...
void FaceBoss::Render() {
    ...
    // Mouth rendering
    DrawTexturePro(game->GameResources.Textures["mouth"], {0, 0, 300, 36},
...
void FaceBoss::Update() {
    ...
    if (Distance <= 450 && Health > 0) {
        ...
        this->game->GameMode.CurrentBoss = this;
        this->game->GameMode.CurrentBossName = "The Bouncing Face";
    }
    ...
    if (BossFightStarted) {
        ...
        for (int i = 0; i < array.size(); i++) {
            if (shared_ptr<Spawner> entity = dynamic_pointer_cast<Spawner>(array.at(i)); entity != nullptr and !entity->ShouldDelete) {
                entity->SpawnerIsActive = this->game->GameMode.LevelTimer > 0 ? 999 : 0;
            }
        }
    }
    Entity::Update();
